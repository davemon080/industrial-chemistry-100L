  ...
  const fetchAllData = useCallback(async () => {
    ...
    try {
      ...
      if (email) {
        ...
        // Fetch total users for admins
        const [userCheck] = await sql`SELECT is_admin FROM users WHERE email = ${email} LIMIT 1`;
        if (userCheck?.is_admin || email === 'admin@gmail.com') {
           fetchPromises.push(sql`SELECT COUNT(*) FROM users`);
        }
      }
      ...
      // Handle total users result if present (it's the 5th promise if it exists)
      if (results[4]) {
        setTotalUsers(parseInt(results[4][0]?.count || "0"));
      }
    } catch (e) {
